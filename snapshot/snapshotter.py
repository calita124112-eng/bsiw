...
def list_snapshots():
    folder = "results/snapshot"
    if not os.path.exists(folder):
        print("📂 No snapshots taken yet.")
        return []
    files = [f for f in os.listdir(folder) if f.endswith(".json.gz")]
    if not files:
...
def choose_snapshots():
    ...
    try:
        ...
        if 0 <= first < len(files) and 0 <= second < len(files):
            return os.path.join("results/snapshot", files[first]), os.path.join(
                "results/snapshot", files[second]
            )
        else:
